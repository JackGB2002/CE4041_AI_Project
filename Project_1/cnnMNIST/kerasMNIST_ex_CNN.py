...
#
# Note that here we use the "functional" approach to contructing the Keras model, this 
# is more convenient as the models get bigger.
#
in_shape = training_images.shape[1:]   # Returns s 2-tuple (row,cols) which is the shape of the input images.

model = Sequential()

if NEW_KERAS:
    model.add(InputLayer(shape=in_shape))
    model.add(Conv2D(10, kernel_size=3, padding='same'))
else:
    model.add(Conv2D(10, kernel_size=3, padding='same', input_shape=in_shape))
