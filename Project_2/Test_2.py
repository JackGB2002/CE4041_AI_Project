...
class AdaBoost:
    ...
    def fit(self, X, y):
        ...
        for a in range(self.n_classifiers):
            ...
            print("Accuracy:", self.accuracy(), " Step : ", a)
...
def plot_scatter(X, y):
    ...
    plt.savefig("./Output_Plots/Points.png")
...
def plot_decision_boundary(X, y, model):
    ...
    plt.show()
    os.makedirs("./Output_Plots", exist_ok=True)
    plt.savefig("./Output_Plots/Decision_Boundaries_Ethan.png")
...
if __name__ == "__main__":
    # Load data
    X_train, y_train, X_test, y_test = load_data('./adaboost-train-24.txt','./adaboost-test-24.txt')

    # Plot scatter of training data
    plot_scatter(X_train, y_train)

    # Evaluate model with varying number of classifiers
    n_values = range(1, 100)  # Testing with 1 to 50 classifiers
    train_accuracies = []
    test_accuracies = []


    train_acc, test_acc = evaluate_model(X_train, y_train, X_test, y_test, 1000)
